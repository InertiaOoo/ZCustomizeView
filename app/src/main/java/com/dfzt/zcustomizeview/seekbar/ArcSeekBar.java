package com.dfzt.zcustomizeview.seekbar;
...
public class ArcSeekBar extends View {
    ...
    private Paint progressPaint;  //当前进度弧形
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        progressColors = getColorsByArrayResId(context, R.array.arc_colors_progress);
    ...
    private void initProgressPaint() {
        ...
        progressPaint.setStrokeWidth(mArcWidth + mBorderWidth * 2);
...
//        mArcPaint.setColor(0x1C1B20);
    ...
    private void drawBackground(Canvas canvas) {
        ...
        //设置渐变色
        SweepGradient sweepGradient = new SweepGradient(mCenterX, mCenterY, progressColors, null);
        Matrix matrix = new Matrix();
        matrix.setRotate(180, mCenterX, mCenterY);
        sweepGradient.setLocalMatrix(matrix);
        progressPaint.setShader(sweepGradient);
        canvas.drawArc(content, mOpenAngle / 2 + currentAngle, 360 - mOpenAngle - currentAngle, false, progressPaint);
    ...
    private void drawViewLine(Canvas canvas) {
       ...
       /* //画一条刻度线
        ...
        //画101条刻度线
        //确定每次旋转的角度
        float rotateAngle = (360 - mOpenAngle) / 90;

        //绘制需要有颜色部分的画笔
        Paint targetLinePatin = new Paint();
        targetLinePatin.setColor(Color.BLACK);
        targetLinePatin.setStrokeWidth((float) 1.5);
        targetLinePatin.setAntiAlias(true);
        //记录已经绘制过的有色部分范围(角度float)

        int imax = (int) (currentAngle / rotateAngle);
        if (currentAngle == 270) {
            imax++;
        }else {
            imax--;
        ...
        for (int i = 0; i < imax; i++) {
            canvas.drawLine(0, radius - mArcWidth - mBorderWidth - mShadowRadius * 2, 0, radius - mBorderWidth - mShadowRadius * 2, targetLinePatin);
//            if (hasDraw <= currentAngle && currentAngle != 0) {
//                //计算已经绘制的比例
//                canvas.drawLine(0, radius - 6-mArcWidth-mBorderWidth, 0, radius - 6-mBorderWidth, targetLinePatin);
//            } else {
//                canvas.drawLine(0, radius-6-mArcWidth, 0, radius-6, targetLinePatin);
//            }
            hasDraw += rotateAngle;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case ACTION_MOVE:
                ...
                float tempProgressPresent1 = getCurrentProgress(event.getX(), event.getY());
                if (!mAllowTouchSkip) {
                    // 不允许突变
                    if (Math.abs(tempProgressPresent1 - mProgressPresent) > 0.5f) {
                        break;
                    }
                }
                // 允许突变 或者非突变
                mProgressPresent = tempProgressPresent1;
                computeThumbPos(mProgressPresent);
                // 事件回调
    ...
    private void computeThumbPos(float present) {
        ...
        currentAngle = (float) present * (360 - mOpenAngle);
